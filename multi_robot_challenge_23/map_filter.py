...
class MapFilterClass(Node):
    def __init__(self):
        ...
        self.marker_msg.lifetime = rclpy.duration.Duration().to_msg() # type: ignore
    ...
    def timer_callback(self):
        map_data = self.map_msg.data
        if not map_data:
            self.get_logger().warn('Map data is empty. Cannot find bounding box')
            self.pub_filtered_map.publish(self.map_msg)
            return
        
        x_min_grid = self.map_msg.info.height
        x_max_grid = -1
        y_min_grid = self.map_msg.info.width
        y_max_grid = -1

        found_wall = False

        for map_filter, value in enumerate(map_data):
            if value == 100:
                found_wall = True
                x_grid, y_grid = self.get_map_pos(map_filter)

                x_min_grid = min(x_min_grid, x_grid)
                x_max_grid = max(x_max_grid, x_grid)
                y_min_grid = min(y_min_grid, y_grid)
                y_max_grid = max(y_max_grid, y_grid)

        if not found_wall:
            self.get_logger().warn('No occupied cells (value 100) found in the map data.')
            self.pub_filtered_map.publish(self.map_msg)
            return
        
        point_min_min = self.get_world_pos(x_min_grid, y_min_grid)
        
        y_max_grid_outer = y_max_grid + 1
        point_min_max = self.get_world_pos(x_min_grid, y_max_grid_outer)

        x_max_grid_outer = x_max_grid + 1
        point_max_min = self.get_world_pos(x_max_grid_outer, y_min_grid)

        point_max_max = self.get_world_pos(x_max_grid_outer, y_max_grid_outer)

        self.marker_msg.points = [point_min_min, point_min_max, point_max_min, point_max_max]
